...
# ---------------------------------------------------------------------------
# Unit: stub subcommand
# ---------------------------------------------------------------------------

class TestStubCommand:
    def test_stub_writes_file(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from typer.testing import CliRunner
        from cli.main import app

        runner = CliRunner()
        result = runner.invoke(app, ["stub", "src/payments/invoice.py"])
        assert result.exit_code == 0
        # typer.echo(test_path) is the last non-empty line; Rich progress goes earlier
        stub_path = [ln for ln in result.output.splitlines() if ln.strip()][-1].strip()
        assert stub_path.endswith(".py")
        assert Path(tmp_path / stub_path).exists()

    def test_stub_no_write_prints_content(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from typer.testing import CliRunner
        from cli.main import app

        runner = CliRunner()
        result = runner.invoke(app, ["stub", "src/auth/login.py", "--no-write"])
        assert result.exit_code == 0
        assert "import pytest" in result.output
        assert "auto-generated by quality-orchestrator" in result.output

    def test_stub_creates_parent_dirs(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from typer.testing import CliRunner
        from cli.main import app

        runner = CliRunner()
        result = runner.invoke(app, ["stub", "src/deeply/nested/module.py"])
        assert result.exit_code == 0
        stub_path = [ln for ln in result.output.splitlines() if ln.strip()][-1].strip()
        assert Path(tmp_path / stub_path).parent.exists()

    def test_stub_invalid_framework_exits(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from typer.testing import CliRunner
        from cli.main import app

        runner = CliRunner()
        result = runner.invoke(app, ["stub", "src/foo.py", "--framework", "bogus"])
        assert result.exit_code != 0
